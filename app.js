...
if (verifyIdBtn) {
    verifyIdBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (isIdAlreadyTaken) {
                alert("SECURITY ALERT: This Student ID is already registered to another account!");
    ...
    chatImageInput.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                canvas.toBlob((blob) => {
                    uploadMediaToFirebase(new File([blob], file.name, { type: 'image/jpeg', lastModified: Date.now() }), file.name, 'image');
                }, 'image/jpeg', 0.7);
            };
        };
    });
...
function openChatRoom(facultyName) {
    ...
    onSnapshot(q, async (snapshot) => {
        ...
        msgs.forEach(msg => {
            ...
            // Added touch/mouse events for Long Press Selection directly on the row
            html += `
